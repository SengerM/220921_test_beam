...
def jitter_calculation_test_beam(bureaucrat:RunBureaucrat, signals_names:set, CFD_thresholds='best', force:bool=False):
	"""Calculates the jitter from a test beam between two devices.
	
	Arguments
	---------
	bureaucrat: RunBureaucrat
		A bureaucrat to handle this run.
	signals_names: set of str
		A set with the names of the signals to use to calculate the jitter.
	...
	if len(signals_names) != 2:
		raise ValueError(f'`signals_names` must contain exactly two signal names.')
	
	TASK_NAME = 'jitter_calculation_test_beam'
	...
	with Norberto.handle_task(TASK_NAME) as Norbertos_employee:
		...
		if Norberto.check_these_tasks_were_run_successfully('clean_test_beam', raise_error=False): # If there was a cleaning done, let's take it into account...
			shutil.copyfile( # Put a copy of the cuts in the output directory so there is a record of what was done.
				Norberto.path_to_directory_of_task('clean_test_beam')/Path('cuts.backup.csv'),
		...
		data_df = data_df.join(signal_names, on='slot_number')

		if any([sn not in set(data_df['signal_name']) for sn in signals_names]):
			raise ValueError(f'One (or the two) of `signals_names` {signals_names} is not present in the data...')
		data_df = data_df.query(f'signal_name in {list(signals_names)}')
...
def jitter_calculation_test_beam_sweeping_voltage(bureaucrat:RunBureaucrat, signals_names:set, CFD_thresholds='best', force_calculation_on_submeasurements:bool=False, number_of_processes:int=1):
	...
	with multiprocessing.Pool(number_of_processes) as p:
		p.starmap(
			jitter_calculation_test_beam,
			[(bur,sgn_nms,thrshld,frc) for bur,thrshld,frc,sgn_nms in zip(subruns, [CFD_thresholds]*len(subruns), [force_calculation_on_submeasurements]*len(subruns), [signals_names]*len(subruns))]
		)
...
def script_core(bureaucrat:RunBureaucrat, signals_names, CFD_thresholds, force:bool=False):
	Nestor = bureaucrat
	if Nestor.was_task_run_successfully('test_beam_sweeping_bias_voltage'):
		jitter_calculation_test_beam_sweeping_voltage(
			bureaucrat = Nestor,
	...
	elif Nestor.was_task_run_successfully('test_beam'):
		jitter_calculation_test_beam(
			bureaucrat = Nestor,
			signals_names = signals_names,
			CFD_thresholds = CFD_thresholds,
